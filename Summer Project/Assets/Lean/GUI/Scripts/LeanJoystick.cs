...
namespace Lean.Gui
{
	...
	public class LeanJoystick_Inspector : LeanInspector<LeanJoystick>
	{
		protected override void DrawInspector()
		{
			...
			{
				EditorGUI.indentLevel++;
					Draw("dampening", "This allows you to control how quickly the joystick handle position updates (-1 = instant).\nNOTE: This is for visual purposes only, the actual joystick <b>ScaledValue</b> will instantly update.");
					Draw("snapWhileHeld", "If you only want the smooth <b>Dampening</b> to apply when the joystick is returning to the center, then you can enable this.");
				EditorGUI.indentLevel--;
			}

			EditorGUILayout.Separator();

			Draw("downTransitions", "This allows you to perform a transition when a finger begins touching the joystick.\nYou can create a new transition GameObject by right clicking the transition name, and selecting <b>Create</b>.\nFor example, the <b>Graphic.color Transition (LeanGraphicColor)</b> component can be used to change the color.\nNOTE: Any transitions you perform here must be reverted in the <b>Up Transitions</b> setting using a matching transition component.");
...
namespace Lean.Gui
{
	...
	public class LeanJoystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
	{
		...
		/// <summary>This allows you to control how quickly the joystick handle position updates (-1 = instant).
		...
		public void OnPointerDown(PointerEventData eventData)
		{
			if (pointer == null)
			{
				...
				if (relativeToOrigin == false)
				{
					...
					origin = RectTransformUtility.WorldToScreenPoint(null, worldPoint);
				}
